...
static node_t *new_node(void *data)
{
	...
	devInfoTable_t *devdata = (devInfoTable_t *)malloc(sizeof(devInfoTable_t));
	memcpy(devdata,data,sizeof(devInfoTable_t));
    node->data = devdata; 
...
int list_pop_tail(list_t *list)
{
    ...
    if (!(head->next)){ //only 1 node , pop it.
		memset(head->data,0,sizeof(devInfoTable_t));
		free(head->data);
	...
	memset(head->next->data,0,sizeof(devInfoTable_t));
    free(head->next->data);
	memset(head->next,0,sizeof(node_t));
    // free(temp); 
    free(head->next); 
    head->next = NULL; 
    return 1; 
...
int list_pop(list_t *list, int pos)
{
    ...
    head->next = temp->next; 
	// devdata = temp->data;
	memset(temp->data,0,sizeof(devInfoTable_t));
    free(temp->data);
...
int list_index_by_devID(list_t *list, void *data)
{
    ...
    for (; t; t = t->next, i++){
		printf("\n The data1 is %s\n",t->data->devId);
		printf("\n The data2 is %s\n",(uint8_t*)(((devInfoTable_t*)data)->devId));
		printf("\n The index is %d\n",i);
		
		if (strcmp(t->data->devId,(uint8_t*)(((devInfoTable_t*)data)->devId)) == 0) return i;
        // if (t->data == data) return i; 
    }
    return -1; 
}
